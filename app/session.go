...
// SetSessionExpireInHours sets the session's expiry the specified number of minutes
// relative to either the session creation date or the current time, depending
// on the `ExtendSessionOnActivity` config setting.
func (a *App) SetSessionExpireInHours(session *model.Session, minutes int) {
	a.ch.srv.userService.SetSessionExpireInHours(session, minutes)
}
...
func (a *App) createSessionForUserAccessToken(tokenString string) (*model.Session, *model.AppError) {
	...
	a.ch.srv.userService.SetSessionExpireInHours(session, model.SessionUserAccessTokenExpiry)
